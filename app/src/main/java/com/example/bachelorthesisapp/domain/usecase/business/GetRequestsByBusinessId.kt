...
class GetRequestsByBusinessIdUseCase @Inject constructor(
    private val requestsRepository: AppointmentRequestsRepository
): UseCase<GetRequestsByBusinessIdUseCaseArgs, List<AppointmentRequestDto>> {

    override suspend fun execute(
        args: GetRequestsByBusinessIdUseCaseArgs,
        ...
        when (val resource = requestsRepository.getRequestsByBusinessId(businessId = args.businessId)){
            is Resource.Success -> {
                successCallback(resource.value.filter { it.status == RequestStatus.Pending.name })
            }
...
data class GetRequestsByBusinessIdUseCaseArgs(
